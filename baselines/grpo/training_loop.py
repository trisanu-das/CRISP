"""Training loop for GRPO baselines.

This script mirrors the CRISP training loop structure, but uses the GRPO step
implementation from baselines/grpo.py instead of the CRISP objective.

Supported variants:
- k=1  (degenerate / low-rollout control)
- k=8  (stronger baseline)

The goal is to keep all non-objective components identical to CRISP:
- config loading
- dataset building
...
- optimizer setup
- checkpointing
- optional evaluation and WandB logging
"""
...
from .training_step import grpo_step, grpo_step_k1, grpo_step_k8, GRPOConfig
...
def build_train_dataset(config: Mapping[str, Any]):
    """Build the training dataset from the config.

    This mirrors the CRISP training pipeline and yields rows with at least
    `prompt` and `answer`.
    """
...
def init_wandb(config: Mapping[str, Any]):
    ...
    return wandb.init(
        ...
        config=config,
        name=deep_get(config, "experiment.name", "grpo_baseline"),
        dir=deep_get(config, "experiment.output_dir", "runs/grpo_baseline"),
...
def train(config_or_path: str | Mapping[str, Any]) -> Dict[str, Any]:
    ...
    output_dir = Path(exp_cfg.get("output_dir", "runs/grpo_baseline"))
    ...
    k = int(config.get("baselines", {}).get("grpo_k8", {}).get("k", config.get("grpo", {}).get("k", 8)))
    grpo_cfg = GRPOConfig(
        k=k,
        ...
        clip_ratio=float(config.get("training", {}).get("clip_ratio", 0.2) or config.get("crisp", {}).get("clip_ratio", 0.2)),
        advantage_eps=float(config.get("crisp", {}).get("advantage_eps", 1e-8)),
        reward_centering=bool(config.get("crisp", {}).get("reward_centering", True)),
        reward_scaling="group_standardize",
        max_prompt_length=config.get("data", {}).get("max_prompt_length"),
    )

    step_fn = grpo_step_k1 if grpo_cfg.k == 1 else grpo_step_k8 if grpo_cfg.k == 8 else grpo_step
    ...
    for step in range(total_steps):
        ...
        logs = step_fn(model, tokenizer, batch, grpo_cfg, step=step, optimizer=optimizer, device=device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train GRPO baseline")
