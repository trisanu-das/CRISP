...
def compute_rewards(generations: Sequence[str], batch: Sequence[Mapping[str, str]]) -> Tensor:
    rewards = torch.zeros(len(generations), dtype=torch.float32)
...
def opsd_step(
    ...
    *,
    ...
    student_inputs = _tokenize(tokenizer, student_prompts, device=device)
    teacher_inputs = _tokenize(tokenizer, teacher_prompts, device=device)
    ...
    if cfg.use_on_policy_rollout:
        ...
        rewards = compute_rewards(generated_texts, batch)
    else:
        generated_ids = None
