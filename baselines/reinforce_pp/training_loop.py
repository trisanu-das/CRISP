"""Training loop for the REINFORCE++ baseline.

This mirrors the CRISP / GRPO training harness, but uses the critic-free
REINFORCE++ objective defined in baselines/reinforce_pp.py.
...
from .training_step import reinforce_pp_step, ReinforcePPConfig
...
def init_wandb(config: Mapping[str, Any]):
    ...
    return wandb.init(
        ...
        config=config,
        name=deep_get(config, "experiment.name", "reinforce_pp_baseline"),
        dir=deep_get(config, "experiment.output_dir", "runs/reinforce_pp_baseline"),
...
def train(config_or_path: str | Mapping[str, Any]) -> Dict[str, Any]:
    ...
    output_dir = Path(exp_cfg.get("output_dir", "runs/reinforce_pp_baseline"))
    ...
    reinforce_cfg = ReinforcePPConfig(
        ...
        clip_ratio=float(config.get("crisp", {}).get("clip_ratio", 0.2)),
        ...
        reward_scaling=str(config.get("crisp", {}).get("reward_scaling", "standardize")),
    ...
    for step in range(total_steps):
        ...
        logs = reinforce_pp_step(model, tokenizer, batch, reinforce_cfg, step=step, optimizer=optimizer, device=device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train REINFORCE++ baseline")
