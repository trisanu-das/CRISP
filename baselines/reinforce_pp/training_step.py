...
def compute_rewards(batch_rollouts: Sequence[str], batch: Sequence[Mapping[str, str]]) -> Tensor:
    rewards = torch.zeros(len(batch_rollouts), dtype=torch.float32)
...
def reinforce_pp_step(
    ...
    *,
    ...
    inputs = _tokenize(tokenizer, prompts, device=device)
