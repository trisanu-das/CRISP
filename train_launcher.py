...
Examples:
    ...
    python train_launcher.py crisp --config config/crisp_7b.yaml --lambda-max 0.5 --pc-grad on

For a Kaggle-scale pilot, swap --config for config/crisp_kaggle.yaml.
"""
...
def run_target(target: str, cfg: Dict[str, Any]) -> None:
    if target == "crisp":
        ...
        from baselines.grpo.training_loop import train
        train(cfg)
    elif target == "reinforce_pp":
        from baselines.reinforce_pp.training_loop import train
        train(cfg)
    elif target == "opsd":
        from baselines.opsd.training_loop import train
        train(cfg)
    else:
