    ...
    *,
    ...
    if isinstance(config, CrispStepConfig):
        ...
        cfg = CrispStepConfig(
            ...
            max_answer_length=data_cfg.get("max_answer_length"),
        )
    ...
    student_inputs_grad = _tokenize_texts(tokenizer, student_texts, device=device, max_length=cfg.max_prompt_length)
    ...
    # -------------------------
    # Teacher (privileged context) and student (prompt-only) share weights and
    ...
    sd_combined_inputs = _tokenize_texts(tokenizer, sd_combined_texts, device=device, max_length=sd_max_length)
    sd_combined_out = model(**sd_combined_inputs)
    sd_combined_logits = sd_combined_out.logits

    student_logits = sd_combined_logits[:n_items]
    teacher_logits = sd_combined_logits[n_items:]

    # Both were padded together above so lengths already match; slicing to the
    # shared min length is kept as a defensive no-op in case of future changes.
    ...
    # NOTE: torch.autograd.grad's kwarg is `retain_graph`, not `retain` (the
    # previous `retain=True` here was an invalid kwarg that raised a TypeError
    # on every single call). retain_graph=False is correct for the SD term
    # specifically: sd_combined_out's graph isn't reused after this, so
    # releasing it immediately also reduces peak memory.
    g_rl = torch.autograd.grad(l_rl, params, retain_graph=True, allow_unused=True)
    g_sd = torch.autograd.grad(l_sd, params, retain_graph=False, allow_unused=True)
